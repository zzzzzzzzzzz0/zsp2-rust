	...
	let i = cp.for__(&mut args.clone().into_iter(), |tag, argv, _item, _i3| {
		match tag {
			...
			"程序2" => {
				...
				if let Err((i, s, s2)) = exec.hello2__(argv, env) {
					t_::err__(i, s, s2, &mut err);
					return 3
				}
				match exec.obj__(&as_ref__!(env.ret), 0, |obj, _ret2, _end| {
					if let Some(mut stdout1) = obj.o_.as_mut() {
						...
						/*thread::spawn(move || {
							...
							loop {
								match stdout.read_line(&mut s) {
									Ok(siz) => {
										if siz == 0 {
											if f__(t_::eval__(&over_src, &t_::env2__())) {}
											break
										}
									}
									Err(e) => eprintln!("{}", e)
			...
			"背解" | "解背" => {
				...
				if thread4__(&zs_::code_::Env_::new2(zs_::t__(q), env)).is_ok() {}
			}
			_ => {}
		}
		0
...
#[no_mangle]
extern fn thread4__(env:&zs_::code_::Env_) -> zs_::Result2_ {
	thread4_::start__(env)
}
